...
describe('Municípios (e2e)', () => {
  ...
  it('GET /api/municipios devolve itens e total, sem id interno', async () => {
    const resposta = await request(app.getHttpServer()).get('/api/municipios').expect(200);
  ...
  it('recusa parâmetro de consulta não declarado', async () => {
    await request(app.getHttpServer()).get('/api/municipios?uf=SP').expect(400);
  });

  it('recusa limite acima do teto', async () => {
    await request(app.getHttpServer()).get('/api/municipios?limite=100000').expect(400);
  });

  it('recusa nome com caractere de injeção', async () => {
    await request(app.getHttpServer())
      .get(`/api/municipios?nome=${encodeURIComponent("'; DROP TABLE municipio; --")}`)
      .expect(400);
  });

  it('recusa código IBGE não numérico', async () => {
    await request(app.getHttpServer()).get('/api/municipios/abc').expect(400);
  });
  ...
  it('não vaza detalhe interno quando o banco falha', async () => {
    ...
    const resposta = await request(app.getHttpServer()).get('/api/municipios').expect(500);
